function out = run_im2mcg(imageDir,imageName,method)
...
% run install.m first!!!!!!!
tic;
I = imread(fullfile(imageDir,imageName));

if(all(method == 'fast'))
    % Test the 'fast' version, which takes around 5 seconds in mean
    [candidates, mcg] = im2mcg(I,'fast',1);
elseif(all(method=='acurate'))
    % Test the 'accurate' version, which tackes around 20 seconds in mean
    [candidates, mcg] = im2mcg(I,'accurate',1);
end
masks = candidates.masks;
save('matlabOut.mat','masks');
out = 1; %candidates.masks;
toc;
