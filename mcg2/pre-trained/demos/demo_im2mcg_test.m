...
[candidates_scg, ucm2_scg] = im2mcg(I,'fast',1);
toc;

tic;
% Test the 'accurate' version, which tackes around 20 seconds in mean
[candidates_mcg, ucm2_mcg] = im2mcg(I,'accurate',1);
toc;

image_section = zeros(size(I),'uint8');
for i=1:20
    %copy correct values to array
    for k = 1:3
        image_section(:,:,k) = (uint8((candidates_mcg.superpixels==i)).*I(:,:,k));
    end
    figure()
