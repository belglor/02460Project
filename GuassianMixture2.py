...
def gaussian_fun(x, u, sigma):
    ...
    x_mu = x - u
    return np.exp(-(np.dot(x_mu, np.dot(inv(sigma), x_mu))) / 2.0) /  \
        (((2 * np.pi)**(x.shape[0] / 2.0)) * (det(sigma) ** 0.5))
...
def gmm(X, K, max_iter=10, eps=1e-6):
    ...
    for i in range(max_iter):
        # E-step
        gamma = np.apply_along_axis(lambda x: np.fromiter(
            (pi[k] * gaussian_fun(x, mu[k], sigma[k]) for k in range(K)), dtype=float), 1, X)
        ...
        sigma = np.sum(gamma[Ellipsis, np.newaxis, np.newaxis] * x_mu[:, :, np.newaxis, :]
                       * x_mu[:, :, :, np.newaxis], 0) / Nk[Ellipsis, np.newaxis, np.newaxis]
        pi = Nk / N

        # Likelihood
        Lnew = np.sum(np.log2(np.sum(np.apply_along_axis(lambda x: np.fromiter(
            (pi[k] * gaussian_fun(x, mu[k], sigma[k]) for k in range(K)), dtype=float), 1, X), 1)))
        if abs(L - Lnew) < eps:
            break
        L = Lnew
        print("L=%s" % L)
